...
function Home({ selectedSet = {} }) {
  ...
  const handleSave = () => {
    ...
    const collection = firestoreRef.collection("results");
    collection.doc(generateUuid()).set(
      {
        date: dateParser(),
        doneQuestions,
        userName,
        setUuid: selectedSet.uuid,
      ...
      {existedSet && (
        <Box>
          ...
          {flowStep === 2 && (
            <Box className={classes.mainBox}>
              <Typography className={classes.mainText} variant="subtitle2">
                Press button to start questionnaire
              </Typography>
              <Box className={classes.editControls}>
                <Button
                  onClick={toNextStep}
          ...
          {flowStep === 4 && (
            <Box className={classes.mainBox}>
              ...
              {resultSuccessMessage.map((message, index) => (
                <Typography key={`successMessage${index}`} variant="subtitle2">
                  {message}
                </Typography>
              ))}
              <Box className={classes.editControls}>
